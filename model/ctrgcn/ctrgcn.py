...
class CTRGC(nn.Module):
    ...
    def forward(self, x, A=None, alpha=1):
        x1, x2, x3 = self.conv1(
            x).mean(-2), self.conv2(x).mean(-2), self.conv3(x)
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.num_point = self.graph.A.shape[1]
        self.data_bn = nn.BatchNorm1d(num_person * in_channels * self.num_point)

        base_channel = 64
        self.l1 = TCN_GCN_unit(in_channels, base_channel,
                               A, residual=False, adaptive=adaptive)
        self.l2 = TCN_GCN_unit(
            base_channel, base_channel, A, adaptive=adaptive)
        self.l3 = TCN_GCN_unit(
            base_channel, base_channel, A, adaptive=adaptive)
        self.l4 = TCN_GCN_unit(
            base_channel, base_channel, A, adaptive=adaptive)
        self.l5 = TCN_GCN_unit(base_channel, base_channel*2,
                               A, stride=2, adaptive=adaptive)
